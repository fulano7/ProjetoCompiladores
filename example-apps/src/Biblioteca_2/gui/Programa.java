...
import negocios.exceptions.ItemNaoEncontradoException;
import negocios.exceptions.RealizarLoginException;
import negocios.exceptions.RepositorioInvalidoException;
import negocios.exceptions.endereco.EnderecoInvalidoException;
import negocios.exceptions.livro.LivroComEmprestimoException;
import negocios.exceptions.livro.LivroIndisponivelException;
import negocios.exceptions.livro.LivroInvalidoException;
import negocios.exceptions.livro.LivroJaCadastradoException;
import negocios.exceptions.livro.LivroNaoEncontradoException;
import negocios.exceptions.pessoa.AlunoComEmprestimoException;
import negocios.exceptions.pessoa.CpfInvalidoException;
import negocios.exceptions.pessoa.MultaGeradaException;
...
import negocios.exceptions.pessoa.TelefoneInvalidoException;
import negocios.exceptions.sala.SalaComEmprestimoException;
import negocios.exceptions.sala.SalaIndisponivelException;
import negocios.exceptions.sala.SalaInvalidaException;
...
public class Programa {
	...
	public static void main(String[] args) {
		...
		try {
			...
			// Possível exceção que pode ser lançada ao iniciar o sistema
			// quem lança essa exceção é o repositorio quando insere
			// o admin
	...
	public static Principal exibirMenuAcao(Principal fachada, boolean first) {
		...
		message += "\nescolha uma opção:\n";
		message += "\t1 - Emprestimo/Devolução\n";
		message += "\t2 - Cadastro\n";
		message += "\t3 - Atualização\n";
		message += "\t4 - Remorção\n";
		message += "\t5 - Relatório\n";
		message += "\t6 - Procurar\n";
		message += "\t7 - Sair";
		...
		case 7: {
			// Sair
			// Salvar informações no repositorio.. caso seja ARQUIVO
			break;
		}
		}
		return fachada;
	...
	private static Principal opcaoAlunoSala(Principal fachada) {
		String message = "";
		message += "escolha uma opção:\n";
		message += "\t1 - Realizar Empréstimo\n";
		message += "\t2 - Realizar Devolução\n";
		message += "\t3 - Voltar\n";
	...
	private static Principal opcaoCadastro(Principal fachada) {
		...
		case 1: {
			...
			String numero = askInfo("[Endereco]Informe o número:");
		...
		case 2: {
			// Livro
			String codigo = askInfo("Digite o código de identificação:");
			String titulo = askInfo("Digite o título:");
		...
		case 4: {
			...
			String codigo = askInfo("Informe um código:");
	...
	private static Principal opcaoAtualizar(Principal fachada) {
		...
		case 1: {
			...
			try {
				...
				message = "Deseja atualizar alguma informação?";
		...
		case 2: {
			// Livro
			String codigo = askInfo("Informe o código do livro:");
			try {
		...
		case 4: {
			String codigo = askInfo("Informe o código da sala:");
	...
	private static Principal opcaoAtualizarMenuAluno(Principal fachada) {
		...
		case 3: {
			// numero
			String info = askInfo("Informe um novo número:");
			try {
				fachada.atualizarInfoAluno(senhaFuncionario, info, menu);
			} catch (SenhaInvalidaException e) {
				System.out.println(e.getMessage());
	...
	private static Principal opcaoAtualizaMenuLivro(Principal fachada) {
		...
		case 1: {
			// Título
			String info = askInfo("Informe um novo título:");
			try {
	...
	private static Principal opcaoRelatorioAluno(Principal fachada) {
		String message = "";
		message += "escolha uma opção como parâmetro de ordenação:\n";
		message += "\t1 - Nome\n";
		message += "\t2 - CPF\n";
		message += "\t3 - Quantidade de Empréstimos\n";
		message += "\t4 - Voltar\n";
	...
	private static Principal opcaoRelatorioLivro(Principal fachada) {
		...
		message += "\t4 - Quantidade de Empréstimos\n";
		message += "\t5 - Voltar\n";
	...
	private static Principal alunoRealizarLoginEmprestimoSala(Principal fachada) {
		String codigoSala = askInfo("Digite o código da sala:");
	...
	private static Principal alunoRealizarLoginEmprestimoLivro(Principal fachada) {
		...
		String codigoLivro = askInfo("Digite o código do livro:");
		String cpfAluno = askInfo("Insira o CPF:");
		String alunoSenha = askInfo("Insira a senha:");
	...
	private static Principal alunoRealizarLoginDevolucaoSala(Principal fachada) {
		String cpfAluno = askInfo("Insira o CPF:");
		String codigo = askInfo("Digite o código da sala:");
	...
	private static Principal opcaoRemover(Principal fachada) {
		...
		case 1: {
			...
			String senhaFuncionario = askInfo("Informe a sua senha para confirmar a ação:");
		...
		case 4: {
			// Funcionario
			String cpf = askInfo("Informe o CPF do Funcionário:");
			String senhaFuncionario = askInfo("Informe a sua senha para confirmar a ação:");
